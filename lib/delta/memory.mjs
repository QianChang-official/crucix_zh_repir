...
export class MemoryManager {
  ...
  isSignalSuppressed(signalKey) {
    ...
    // count=1 (first alert) → tier[0]=0h, count=2 → tier[1]=6h, etc.
    const tierIndex = Math.min(occurrences - 1, ALERT_DECAY_TIERS.length - 1);
    const cooldownHours = ALERT_DECAY_TIERS[Math.max(0, tierIndex)];
