...
**Your own intelligence terminal. 35 sources. One command. Zero cloud.**
...
[![Sources](https://img.shields.io/badge/OSINT%20sources-35-cyan)](#data-sources-35)
...
Crucix pulls satellite fire detection, flight tracking, radiation monitoring, satellite constellation tracking, economic indicators, live market prices, conflict data, sanctions lists, cyber vulnerability feeds, launch schedules, earthquake telemetry, and natural hazard events from 35 open-source intelligence feeds — in parallel, every 15 minutes — and renders everything on a single self-contained Jarvis-style dashboard.
...
The dashboard opens automatically at `http://localhost:3117` and immediately begins its first intelligence sweep. This initial sweep queries all 35 sources in parallel and typically takes 30–60 seconds — the dashboard will appear empty until the sweep completes and pushes the first data update. After that, it auto-refreshes every 15 minutes via SSE (Server-Sent Events). No manual page refresh needed.
...
- **12 marker types** across both views: fire detections, air traffic, radiation sites, maritime chokepoints, SDR receivers, OSINT events, health alerts, geolocated news, conflict events, earthquakes, natural disaster events, launch pads
...
- **Disaster watch** — USGS earthquake telemetry + NASA EONET open natural events on both map modes
- **Cyber threats** — NVD CVEs merged with CISA KEV exploitation intelligence
- **Space watch** — CelesTrak satellite tracking plus Launch Library schedules and Spaceflight News headlines
- **World structural view** — World Bank GDP, inflation, trade exposure, and military-burden snapshots in the macro panel
...
1. Queries all 35 sources in parallel (~30s)
...
Crucix still works with zero API keys. 25+ sources require no authentication at all. Sources that need keys return structured errors and the rest of the sweep continues normally.
...
│
├── apis/
│   ├── briefing.mjs           # Master orchestrator — runs all 35 sources in parallel
...
│   └── sources/               # 35 self-contained source modules
...
- **Parallel execution** — `Promise.allSettled()` fires all 35 sources simultaneously
...
| **World Bank** | GDP, inflation, trade exposure, military burden across major economies | None |

### Tier 3: Weather, Environment, Tech, Social, Hazard (9)
...
| **USGS Earthquake Hazards** | Global earthquakes, tsunami flags, significant quake counts | None |
| **NASA EONET** | Wildfires, storms, volcanoes, floods, sea-ice and other active natural events | None |

### Tier 4: Space & Satellites (3)

| Source | What It Tracks | Auth |
|--------|---------------|------|
| **CelesTrak** | Satellite launches, ISS tracking, military constellations, Starlink/OneWeb counts | None |
| **Launch Library 2** | Upcoming launches, pads, providers, launch probabilities | None |
| **Spaceflight News** | Launch coverage, mission analysis, space-industry headlines | None |
...
| Source | What It Tracks | Auth |
|--------|---------------|------|
| **CISA KEV** | Known exploited vulnerabilities with ransomware flags and due dates | None |
| **Cloudflare Radar** | Internet outages, routing disruption, and traffic anomalies | None |
| **NVD** | Newly published CVEs, severity mix, RCE/auth-bypass indicators | None |
...
This is normal — the first sweep takes 30–60 seconds to query all 35 sources. The dashboard will populate automatically once the sweep completes. Check the terminal for sweep progress logs.
...
Found a bug? Want to add a 36th source? PRs welcome. Each source is a standalone module in `apis/sources/` — just export a `briefing()` function that returns structured data and add it to the orchestrator in `apis/briefing.mjs`.
